...
@media (max-width: 1200px) {
    ...
    <div class="hdr">
      <div class="hdflx">
        ...
        <div class="contdiv"><a href=""><h1>Close window to logout!</h1></a></div>
      </div>
    </div>
      ...
      <div class="details-table">
        <table id="personatable">
          ...
          if(isset($_POST['showdata'])) 
          {
              ...
              $getsel = "all";
              if($_POST['sec_college']==$getsel)
              {
                    $con = mysqli_connect('localhost','exlpuuxq_personanew','exlpuuxq_personanew');
              
                    $query1="SELECT p.UniqueId, p.Name, p.Email, p.Phone, p.College,p.Event,c.Event_name,c.cost,c.host_clg,p.Txnid,p.Time FROM participants p INNER JOIN costs c ON p.Event COLLATE utf8mb4_unicode_ci = c.event";
              ...
              else
              {
                $con = mysqli_connect('localhost','exlpuuxq_personanew','exlpuuxq_personanew');
                    ...
                    $query1="SELECT p.UniqueId, p.Name, p.Email, p.Phone, p.College,p.Event,c.Event_name,c.cost,c.host_clg,p.Txnid,p.Time FROM participants p INNER JOIN costs c ON p.Event COLLATE utf8mb4_unicode_ci = c.event WHERE c.host_clg = '$COLLEGE'";
                    mysqli_select_db($con, 'exlpuuxq_personanew');
                    $result1 = mysqli_query($con,$query1);
                    while($row=mysqli_fetch_assoc($result1))
                      {
